...
public class Compass {
    ...
    public Point rotate(Point point, Direction direction){
        ...
        if (direction == Direction.right && currentIndex != -1) {
            newIndex = (currentIndex + 1) % compassDirections.length; // modulo to loop around
            return compassDirections[newIndex];
        } else if (direction == Direction.left && currentIndex != -1) {
            newIndex = (currentIndex - 1 + compassDirections.length) % compassDirections.length;
            return compassDirections[newIndex];
        }
         ...
         *
         * for the second modulo line, we minus one, and add the length, e.g. giving us from 0, to -1, then adding 4 to get to 3.
